...
# Issue in terminal: python3 s850_generate.py
...
def make_pdf(radius):
    ...
    n = [eval(dn_norm)*(step)*area for s in np.arange(s_min, large_s+step, step)]
    ...
    for i in range(psf_data.shape[0]):
        for j in range(psf_data.shape[1]):
            if 0<y+lb1+i<imgg.shape[0] and 0<x+lb2+j<imgg.shape[1]:
                fake_im[y+i+lb1][x+j+lb2]=scale*psf_data[i][j] 
    return fake_im
    '''
...
def insert(random_num):
    ...
    num, flux, summ = make_pdf(radius) 
    prob = [float(n / summ) for n in num]
    sources = np.random.choice(flux, size=int(summ), p=prob)
    number = len(sources) 
    ...
    for i in range(row): # y axis
        for j in range(col): # x axis 
            if (i-int(row/2))**2+(j-int(col/2))**2 < (radius*30)**2: # due to the shape of the data 
                inj.append((j,i))
    
    positions = random.sample(inj,int(number))
    
    for i in range(len(sources)):
        image_new = image_new + add_source(sources[i],image_new,positions[i])
    '''
    x_pos = np.random.uniform(0,col,20000)
    y_pos = np.random.uniform(0,row,20000)
    positions = []
    positions.append(x_pos)
    positions.append(y_pos)
    positions = np.array(positions).reshape(20000,2)
    positions = np.array([po for po in positions if (po[0]-(col-1)/2)**2+(po[1]-(row-1)/2)**2 < (radius*30)**2])[:len(sources)] # po[0] is x axis and po[1] is y axis
    ...
    #y = np.arange(0,row)
    #x, y = np.meshgrid(x, y)
    #image_new = image_new + inj(x,y) #add_source(sources[i],image_new,positions[i])
    ...
    snr01 = []
    sources1 = []
    #wccs = WCS(fits.open('./mock_850/mock_map'+str(random_num)+'_MF.fits')[2].header)
    #DZ: this method is also correct, but multiple readings might increase the computing time
